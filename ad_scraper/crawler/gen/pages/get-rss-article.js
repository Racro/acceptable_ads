 ...
 */
export default async function getArticleFromRSS(page) {
...
async function getFromHeader(page) {
    log.info(`${page.url()}: Attempting to get RSS feed from header`);
    let rssUrls = await page.evaluate(() => {
        let rss = [];
        let rssLinks = Array.from(document.querySelectorAll('link[rel="alternate"][type="application/rss+xml"]'));
        rss.push(...rssLinks
...
async function guessRssFeed(page) {
...
//# sourceMappingURL=get-rss-article.js.map