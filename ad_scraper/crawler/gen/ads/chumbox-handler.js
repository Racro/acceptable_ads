...
// chumboxes.
export async function splitChumbox(ad) {
    let definitions = [
 ...
 */
async function splitFirstPartyAd(container, linkSelector, parentDepth) {
    let link = await container.$$(linkSelector);
    if (link.length === 0) {
        return null;
    }
    let fullAd = await Promise.all(link.map(async (l) => {
        let parentHandle = await container.evaluateHandle((e, depth) => {
            let current = e;
...
// }
//# sourceMappingURL=chumbox-handler.js.map